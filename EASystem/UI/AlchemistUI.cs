	...
	internal class AlchemistUI : UIState
	{
		...
		public override void OnDeactivate() {
			if (!itemSlot.Item.IsAir) {
				Main.LocalPlayer.QuickSpawnItem(Main.LocalPlayer.GetSource_FromThis(), itemSlot.Item, itemSlot.Item.stack);
		...
		protected override void DrawSelf(SpriteBatch spriteBatch) 
        {
            ...
            DynamicSpriteFont curfont = FontAssets.MouseText.Value;
			...
			if (!itemSlot.Item.IsAir) 
            {
				...
				ChatManager.DrawColorCodedStringWithShadow(Main.spriteBatch, curfont, costText, new Vector2(slotX + 50, slotY), new Color(Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor), 0f, Vector2.Zero, Vector2.One, -1f, 2f);
				ChatManager.DrawColorCodedStringWithShadow(Main.spriteBatch, curfont, coinsText, new Vector2(slotX + 50 + FontAssets.MouseText.Value.MeasureString(costText).X, (float)slotY), Color.White, 0f, Vector2.Zero, Vector2.One, -1f, 2f);
                #endregion
				...
				if (hoveringOverReforgeButton) {
                    ...
                    if (Main.mouseLeftRelease && Main.mouseLeft && Main.LocalPlayer.CanAfford(convertPrice, -1))
                    {
                        ItemLoader.PreReforge(itemSlot.Item);
                        ...
                        PopupText.NewText(PopupTextContext.RegularItemPickup, itemSlot.Item, itemSlot.Item.stack, noStack: false);
				...
				string message = $"{GetInstance<EALocalization>().AlchemistUI}\n{GetInstance<EALocalization>().AlchemistUI1}";
				ChatManager.DrawColorCodedStringWithShadow(Main.spriteBatch, curfont, message, new Vector2(slotX + 50, slotY), new Color(Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor), 0f, Vector2.Zero, Vector2.One, -1f, 2f);
			}
		}
	}
}
