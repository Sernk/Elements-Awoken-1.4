...
namespace ElementsAwoken
{
    public class ElementsAwoken : Mod
    {
            ...
            for (int heartNum = 1; heartNum < (int)(player.statLifeMax2 / lifePerHeart) + 1; heartNum++)
            {
                ...
                if (!player.ghost)
                {
                    if (heartsPlayers.chaosHeartLife > 0)
                    {
                        ...
                        //Texture2D heart4Texture = ModContent.Request<Texture2D>("ElementsAwoken/Extra/Heart4").Value;
        ...
        public static void DrawShield()
        {
            ...
            heartsPlayers.shieldLife = modPlayer.shieldHearts; // multiply by however many hearts you want it to make when you use one
            if (heartsPlayers.shieldLife < 0)
            {
                heartsPlayers.shieldLife = 0;
            }
            if (heartsPlayers.shieldLife > 0)
            {
                lifeForHeart = player.statLifeMax / (20 + heartsPlayers.shieldLife / 4);
                lifePerHeart = player.statLifeMax / 20f;
            }
        ...
        public static void DrawHeatBar(Item item)
        {
            ...
            Texture2D backgroundTexture = ModContent.Request<Texture2D>("ElementsAwoken/Extra/HeatBarUI").Value;
            ...
            Texture2D barTexture = ModContent.Request<Texture2D>("ElementsAwoken/Extra/HeatBar").Value;
            ...
            if (!Main.mouseText)
            {
                if (Main.mouseX > UIPos.X && Main.mouseX < UIPos.X + UISize.X && Main.mouseY > UIPos.Y && Main.mouseY < UIPos.Y + UISize.Y)
                {
                    ...
                    ChatManager.DrawColorCodedStringWithShadow(Main.spriteBatch, FontAssets.MouseText.Value, mouseText, new Vector2(Main.mouseX + 17, Main.mouseY + 17), new Color(Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor), 0f, Vector2.Zero, Vector2.One, -1f, 2f);
                }
            }
        }
        ...
        {
            //Mod mod = ModLoader.GetMod("ElementsAwoken");
            //Texture2D UITex = ModContent.Request<Texture2D>("ElementsAwoken/Extra/InsanityUIIcon").Value;
            //Player player = Main.player[Main.myPlayer];
        ...
        {
            //Mod mod = ModLoader.GetMod("ElementsAwoken");
            //Texture2D UITex = ModContent.Request<Texture2D>("ElementsAwoken/Extra/EnergyUIIcon").Value;
            //Player player = Main.player[Main.myPlayer];
            //var modPlayer = player.GetModPlayer<PlayerEnergy>();
            //if (player.ghost || modPlayer.maxEnergy == 0)
            //{
