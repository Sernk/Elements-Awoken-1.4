...
using ElementsAwoken.Content.Projectiles.NPCProj.Ancients.Kirvein;
...
namespace ElementsAwoken.Content.NPCs.Bosses.Ancients
{
    ...
    public class Kirvein : ModNPC
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            {
                CustomTexturePath = "ElementsAwoken/Extra/Bestiary/KirveinBestiary"
            };
            value.Position.X += 0;
            value.Position.Y -= 0f;
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            bestiaryEntry.Info.AddRange([
                ...
                new FlavorTextBestiaryInfoElement("Mods.ElementsAwoken.Bestiary.Bosses.Kirvein"),
            ...
            else
            {
                ...
                if (NPC.life > NPC.lifeMax * 0.3f)
                {
                    if (NPC.ai[2] <= 0)
                    {
                        ...
                        Projectile.NewProjectile(EAU.NPCs(NPC), NPC.Center.X, NPC.Center.Y, (float)((Math.Cos(rotation) * projSpeed) * -1), (float)((Math.Sin(rotation) * projSpeed) * -1), ModContent.ProjectileType<KirveinArrow>(), projectileBaseDamage, 5f, Main.myPlayer);
                ...
                if (specialTimer <= 0)
                {
                    ...
                    {
                        Projectile.NewProjectile(EAU.NPCs(NPC), NPC.Center.X, NPC.Center.Y, 0f, -9f, ModContent.ProjectileType<KirveinFragCrystal>(), projectileBaseDamage, 5f, Main.myPlayer);
                    }
                    if (NPC.life > NPC.lifeMax * 0.6f)
                    {
                        specialTimer = 700;
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            if (NPC.life <= 0)
            {
                for (int i = 0; i < 2; i++)
                {
                    Projectile.NewProjectile(EAU.NPCs(NPC), NPC.Center.X, NPC.Center.Y, 0f, 0f, ModContent.ProjectileType<KirveinShard>(), 0, 0f, Main.myPlayer, i);
                }
                for (int k = 0; k < 80; k++)
                {
                    int dust = Dust.NewDust(NPC.position + NPC.velocity, NPC.width, NPC.height, ModContent.DustType<AncientGreen>(), NPC.oldVelocity.X * 0.5f, NPC.oldVelocity.Y * 0.5f, 100, default(Color), 2f);
