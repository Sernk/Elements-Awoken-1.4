        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 speed, int type, int damage, float knockback)
        {
            if (player.altFunctionUse != 2)
            {
                ...
                if (Main.rand.Next(3) == 0)
                {
                    ...
                    {
                        Vector2 velocity = new Vector2(speed.X, speed.Y) * 1.5f; // ?
            ...
            {
                Vector2 velocity = new Vector2(speed.X, speed.Y) * 1.5f; // TODO: It could be here
                Projectile proj = Main.projectile[Projectile.NewProjectile(source, position.X, position.Y, velocity.X, velocity.Y, ModContent.ProjectileType<AtaxiaBolt>(), (int)MathHelper.Lerp(damage * 0.3f, damage * 1.7f, altCounter / 300), knockback, player.whoAmI,altCounter)];
            }
            return false;
        }
