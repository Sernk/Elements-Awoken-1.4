    ...
    public class DesolationIV : ModItem
    {
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            if (player.altFunctionUse == 2)
            {
                ...
                else if (mode == 1) text = EALocalization.Desolation3;
                else if (mode == 2) text = EALocalization.Desolation1;
                else if (mode == 3) text = EALocalization.Desolation2;
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 speed, int type, int damage, float knockback)
        {
            if (player.altFunctionUse != 2)
            {
                ...
                else if (mode == 1)
                {
                    ...
                    Projectile.NewProjectile(source, position.X + Main.rand.Next(-20, 20), position.Y + Main.rand.Next(-20, 20), speed.X, speed.Y, type, damage, knockback, player.whoAmI, 0.0f, 0.0f);
                }
                else if (mode == 2)
                {
                    Projectile.NewProjectile(source, position.X, position.Y, speed.X, speed.Y, ModContent.ProjectileType<DesolationIVHeld>(), damage, knockback, player.whoAmI, 0.0f, 0.0f);
                }
                else
                {
