...
{
    public class CrystallineKunaiHostileExplosive : ModProjectile
    {
        ...
        {
            float rotation = MathHelper.ToRadians(360f);
            float numProj = Projectile.ai[0] == 0 ? 8f : 3f;
            int projType = Projectile.ai[0] == 0 ? ModContent.ProjectileType<CrystallineShard>() : ModContent.ProjectileType<CrystallineShardHoming>();
            float speed = Projectile.ai[0] == 0 ? 4f : 7f;
            for (int i = 0; i < numProj; i++)
            {
                Vector2 perturbedSpeed = new Vector2(1f, 1f).RotatedBy((rotation / numProj) * i) * speed;
                Projectile proj = Main.projectile[Projectile.NewProjectile(EAU.Proj(Projectile), Projectile.Center.X, Projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, projType, Projectile.damage, 2f, 0)];
