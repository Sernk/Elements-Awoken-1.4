...
{
    public class AquaticBlast : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            while ((float)num468 < num467)
            {
                if (Main.rand.Next(2) == 0)
                {
                    ...
                    num469 *= num472;
                    num470 *= num472;
                    int num473 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 111, 0f, 0f, 100, default(Color), 1.5f);
