...
{
    public class BalladOfBalanceP : ModProjectile
    {
        public float shootTimer = 0f;
        public float shootTimer2 = 20f;
            ...
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                for (int i = 0; i < Main.npc.Length; i++)
                {
                    ...
                    if (nPC.active && !nPC.friendly && nPC.damage > 0 && !nPC.dontTakeDamage && Vector2.Distance(Projectile.Center, nPC.Center) <= max)
                    {
                        ...
                        if (shootTimer <= 0)
                        {
                            Vector2 speed = new((float)((Math.Cos(rotation) * Speed) * -1), (float)((Math.Sin(rotation) * Speed) * -1));
                        ...
                        if (shootTimer2 <= 0)
                        {
                            ...
                            Projectile.NewProjectile(EAU.Proj(Projectile), Projectile.Center.X, Projectile.Center.Y, speed.X, speed.Y, ModContent.ProjectileType<DanceOfDarknessShade>(), Projectile.damage / 2, Projectile.knockBack, Projectile.owner);
                            shootTimer2 = Main.rand.Next(30, 90);
                        }
                    }
                }
            }
        }
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) => target.AddBuff(ModContent.BuffType<Glowing>(), 180);
