...
{
    public class TheDestroyerHeld : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] > 10f)
            {
                ...
                for (int k = 0; k < num16 + 1; k++)
                {
                    ...
                    int num20 = Dust.NewDust(vector14 - Vector2.One * 8f, 16, 16, 219, Projectile.velocity.X / 2f, Projectile.velocity.Y / 2f, 0, default(Color), 1f);
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                if ((player.channel && player.CheckMana(player.inventory[player.selectedItem].mana, true, false)) && !player.noItems && !player.CCed)
                {
                    if (Projectile.ai[0] >= chargeLevel3)
                    {
                        ...
                        Projectile.NewProjectile(EAU.Proj(Projectile), Projectile.Center.X, Projectile.Center.Y, vector16.X, vector16.Y, ModContent.ProjectileType<DestroyerLaserFull>(), damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                ...
                else
                {
                    if (Projectile.ai[0] < chargeLevel3)
                    {
                        ...
                        Projectile.NewProjectile(EAU.Proj(Projectile), Projectile.Center.X, Projectile.Center.Y, vector16.X, vector16.Y, ModContent.ProjectileType<DestroyerLaserHalf>(), damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
