...
{
    public class InfinityFireTentacle : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.scale < 1f)
            {
                ...
                while ((float)num892 < Projectile.scale * 10f)
                {
                    ...
                    {
                        int num893 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 6, Projectile.velocity.X, Projectile.velocity.Y, 100, default(Color), 1.1f);
