            ...
            if (player.itemAnimation < player.itemAnimationMax / 3)
            {
                ...
                if (Projectile.localAI[0] == 0f)
                {
                    ...
                    for (int k = 0; (double)k < (double)count; k++)
                    {
                        ...
                        int dust = Dust.NewDust(Projectile.Center, 0, 0, EAU.PinkFlame, 0.0f, 0.0f, 0, new Color(), 1.0f);
                    ...
                    if (player.itemAnimation < player.itemAnimationMax / 2) Projectile.NewProjectile(EAU.Proj(Projectile), Projectile.Center.X + Projectile.velocity.X, Projectile.Center.Y + Projectile.velocity.Y, Projectile.velocity.X * 14.0f, Projectile.velocity.Y * 15.0f, ModContent.ProjectileType<DespairBody>(), Projectile.damage, Projectile.knockBack * 0.85f, Projectile.owner, 0f, 0f);
                }
            }
            if (Main.rand.Next(3) == 0)
            {
                int dust = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, EAU.PinkFlame, Projectile.velocity.X * 0.2f, Projectile.velocity.Y * 0.2f, 200, new Color(), 1.2f);
