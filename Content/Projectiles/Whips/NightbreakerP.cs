...
{
    public class NightbreakerP : ModProjectile
    {
        ...
        {       
            ...
            if (Projectile.ai[0] == timeAlive / 2)
            {
                ...
                {
                    Vector2 startSpeed = new Vector2(Projectile.velocity.X, Projectile.velocity.Y);
                    startSpeed.Normalize();
                    Vector2 perturbedSpeed = new Vector2(startSpeed.X * 5, startSpeed.Y * 5).RotatedBy(MathHelper.Lerp(-rotation, rotation, i / (numProj - 1)));
                    Projectile.NewProjectile(EAU.Proj(Projectile), Projectile.position.X + Projectile.velocity.X, Projectile.position.Y + Projectile.velocity.Y, perturbedSpeed.X, perturbedSpeed.Y, ModContent.ProjectileType<NightbreakerBall>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                }
            }
            ...
            if (Projectile.alpha == 0)
            {
                ...
                {
                    Dust dust = Main.dust[Dust.NewDust(Projectile.position + Projectile.velocity * 2f, Projectile.width, Projectile.height, 14, 0f, 0f, 100, default(Color), 0.4f)];
                    dust.noGravity = true;
                    dust.velocity *= 2f;
                ...
                while ((float)num53 < num52)
                {
                    if (Main.rand.Next(4) == 0)
                    {
                        ...
                        Dust dust2 = Main.dust[Dust.NewDust(position, Projectile.width, Projectile.height, 27, 0f, 0f, 100, default(Color), 1f)];
