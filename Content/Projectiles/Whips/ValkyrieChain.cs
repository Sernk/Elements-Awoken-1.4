...
{
    public class ValkyrieChain : ModProjectile
    {
        ...
        {       
            ...
            if (Projectile.alpha == 0)
            {
                ...
                {
                    Dust dust = Main.dust[Dust.NewDust(Projectile.position + Projectile.velocity * 2f, Projectile.width, Projectile.height, 60, 0f, 0f, 100, new Color(Main.DiscoR, Main.DiscoG, Main.DiscoB), 2f)];
                    dust.noGravity = true;
                    dust.velocity *= 2f;
                ...
                while ((float)num53 < num52)
                {
                    if (Main.rand.Next(4) == 0)
                    {
                        ...
                        Dust dust2 = Main.dust[Dust.NewDust(position, Projectile.width, Projectile.height, 60, 0f, 0f, 100, new Color(Main.DiscoR, Main.DiscoG, Main.DiscoB), 1f)];
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                Projectile.NewProjectile(EAU.Proj(Projectile), target.Center.X, target.Center.Y, Main.rand.Next(-10,10), Main.rand.Next(-10, 10), ModContent.ProjectileType<ValkyrieBolt>(), Projectile.damage, 0, Projectile.owner);
            }
            Projectile.NewProjectile(EAU.Proj(Projectile), target.Center.X, target.Center.Y, 0, 0, ModContent.ProjectileType<RainbowExplosion>(), Projectile.damage, 0, Projectile.owner);
        }
    }
}