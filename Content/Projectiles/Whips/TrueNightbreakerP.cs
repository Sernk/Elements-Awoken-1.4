...
{
    public class TrueNightbreakerP : ModProjectile
    {
        ...
        {       
            ...
            {
                Projectile.NewProjectile(EAU.Proj(Projectile), Projectile.position.X + Projectile.velocity.X, Projectile.position.Y + Projectile.velocity.Y, Projectile.velocity.X * 0.05f, Projectile.velocity.Y * 0.05f, ModContent.ProjectileType<NightbreakerBlade>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
            }
            ...
            if (Projectile.alpha == 0)
            {
                ...
                while ((float)num53 < num52)
                {
                    if (Main.rand.Next(6) == 0)
                    {
                        ...
                        Dust dust3 = Main.dust[Dust.NewDust(position, Projectile.width, Projectile.height, 27, 0f, 0f, 100, default(Color), 1f)];
                        dust3.noGravity = true;
                        dust3.fadeIn = 0.5f;
                        dust3.velocity += Projectile.localAI[0].ToRotationVector2();
                    }
                    num53++;
                }
            }
