    ...
    public class ItemMagnetEntity : ModTileEntity
    {
                ...
                if (Vector2.Distance(sucked.Center, tileCenter) < 800)
                {
                    ...
                    if (Vector2.Distance(sucked.Center, tileCenter) < 30)
                    {
                        for (int chest = 0; chest < 1000; chest++)
                        {
                            ...
                            {
                                if (sucked.type == ItemID.Star || sucked.type == ItemID.Heart || sucked.type == ItemID.NebulaPickup1 || sucked.type == ItemID.NebulaPickup2 || sucked.type == ItemID.NebulaPickup3 || sucked.type == ItemType<Content.Items.Tech.Generators.EnergyPickup>()) break;
                                // adding the items
                                foreach (Item chestItem in currentChest.item)
                                {
                                    if (sucked.IsNotSameTypePrefixAndStack(chestItem)) // IsTheSameAs
                                    {
        ...
        private void ChangeCoinTo(int type, Chest chest, Item coin)
        {
            ...
            **/
            ...
            {
                if (newCoin.IsNotSameTypePrefixAndStack(chestItem))
                {
                    int available = chestItem.maxStack - chestItem.stack;
        ...
        public override bool IsTileValidForEntity(int i, int j)
        {
            ...
            return tile.HasTile && tile.TileType == TileType<ItemMagnet>() && tile.TileFrameX == 0 && tile.TileFrameY == 0;
        }
